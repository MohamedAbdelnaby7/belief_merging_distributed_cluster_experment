...
class TrialTask:
    ...
    def get_task_id(self):
        ...
        return f"{self.pattern}_trial{self.trial_id}_interval{self.merge_interval}_seed{self.trial_seed}"
...
class DistributedExperimentManager:
    """Manages distributed execution with checkpointing"""
...
def run_single_trial_task(task: TrialTask) -> Optional[Dict]:
    ...
    try:
        ...
        experiment = ControlledMergingExperiment(
            grid_size=task.config.grid_size,
        ...
        target_policy = TargetMovementPolicy(task.config.grid_size, task.pattern)
        initial_target = np.random.randint(0, task.config.grid_size[0] * task.config.grid_size[1])
        ...
        total_states = task.config.grid_size[0] * task.config.grid_size[1]
        ...
        initial_positions = np.random.choice(
            available_positions, 
            task.config.n_agents, 
            replace=False
        ).tolist()
        
        # Pre-generate observation random numbers
        observation_randoms = np.random.random((task.config.max_steps, task.config.n_agents))
...
def main():
    ...
    if args.config_file and os.path.exists(args.config_file):
        ...
        # Default configuration with TRUE MPC
        config = ExperimentConfig(
            grid_size=(20, 20),
            n_agents=4,
            alpha=0.1,
